...
if ($remaining_time <= 0) {
    ...
    $part++;
    if($part==5){
        $queryEndExam = $conn->query("UPDATE sessions SET current_part = '$part',exam_end_status=1 WHERE exam_id = '$exam_id' AND examin_id = '$exmne_id';
                                    UPDATE exam_attempt set examat_status = 1 WHERE exam_id = '$exam_id' AND exmne_id = '$exmne_id'");
    }else{
        $queryChangePart = $conn->query("UPDATE sessions SET current_part = '$part' WHERE exam_id = '$exam_id' AND examin_id = '$exmne_id'");
    }
